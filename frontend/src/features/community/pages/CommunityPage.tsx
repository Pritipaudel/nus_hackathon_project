...
const CATEGORIES = ['ALL', 'ANXIETY', 'DEPRESSION', 'STRESS', 'SLEEP', 'RELATIONSHIPS', 'TRAUMA', 'GENERAL'];
...
const MOCK_POSTS: (CommunityPost & { upvotes: number; comments: number })[] = [
  {
    id: '1',
    username: 'anonymous_4821',
    content: 'Three weeks into the anxiety programme and I finally understand what a thought record actually does. The first week I thought it was pointless. Now I can catch the spiral before it takes over. Small win but it matters.',
    media_urls: [MOCK_IMAGES[0]],
    category: 'ANXIETY',
    is_verified: false,
  ...
  },
  {
    ...
    content: 'A reminder for anyone struggling with sleep: your bed should only be for sleep and rest. Working, scrolling, or worrying in bed trains your brain to stay alert there. Start with just this one change — it compounds.',
    media_urls: [],
    category: 'SLEEP',
    is_verified: true,
  ...
  },
  {
    ...
    content: 'The cultural pressure around career choices hits different when your parents see your choices as a reflection of the whole family. Anyone else navigating this? Would appreciate hearing how others have approached the conversation.',
    media_urls: [MOCK_IMAGES[1]],
    category: 'STRESS',
    is_verified: false,
  ...
  },
  {
    ...
    content: 'I have been using the mood tracker for two weeks. Noticing that my low days cluster around Monday and Thursday. No idea why yet but at least I can see a pattern now instead of feeling like it is random.',
    media_urls: [],
    category: 'DEPRESSION',
    is_verified: false,
  ...
  },
  {
    ...
    content: 'Grief does not follow a timeline and it does not follow stages in order. If you are somewhere in a loss right now, whatever you are feeling today is valid — even if it contradicts what you felt yesterday.',
    media_urls: [MOCK_IMAGES[2]],
    category: 'GENERAL',
    is_verified: true,
  ...
  },
  {
    ...
    content: 'Started opening up to my partner about what I go through. It is uncomfortable but it is also the first time in years I do not feel completely alone with it. The community posts here helped me find words for things I could not name before.',
    media_urls: [],
...
const MOCK_TRENDING: TrendingPost[] = [
  { id: '5', content: 'Grief does not follow a timeline and it does not follow stages in order...', trend_score: 203 },
  { id: '1', content: 'Three weeks into the anxiety programme and I finally understand...', trend_score: 134 },
  { id: '3', content: 'The cultural pressure around career choices hits different...', trend_score: 67 },
];
...
const CommunityPage = () => {
  ...
  const [votes, setVotes] = useState<Record<string, 'up' | 'down' | null>>(
    Object.fromEntries(MOCK_POSTS.map((p) => [p.id, null])),
  );
  const [counts, setCounts] = useState<Record<string, number>>(
    Object.fromEntries(MOCK_POSTS.map((p) => [p.id, p.upvotes])),
  );
            ...
            <div key={post.id} className="cp-post">
              ...
              <div className="cp-post__body">
                <div className="cp-post__header">
                  ...
                  <span className={`cp-cat-tag ${CATEGORY_COLORS[post.category] ?? 'cp-cat--gray'}`}>
